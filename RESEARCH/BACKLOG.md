...
- **Status**: in_progress (round-010). *Refinement (round-014)*: Phase A round-012's harness extensions in `src/backtest.py` already support `c_stop=float("inf")` (tested) and `simulate_inventory_aware_sized`. H-024 should now be re-run within Phase A's unified harness against ALL 7 strategies with `c_stop=∞`, not just `baseline_offline_tau`. Multi-strategy DSR (with the same n_trials=137 deflation as round-012) decides whether the no-stop variant rescues *any* combined-signal strategy. The H-304 prioritisation card ranks this as the cheapest test of the binding-constraint hypothesis.
...
- **Mechanism**: at honest val-chosen τ, round-009/Phase A test Sharpe = -0.089 (`baseline_offline_tau`) and ALL 7 Phase-A strategies have multi-strategy DSR=0. Strategy improvement therefore = changing one of four axes: **label / cost / barriers / sizing**. The 2×2 axes-vs-fixedness matrix:
  ...
  **Breakeven hit-rate calculation (load-bearing)**: for the no-stop variant at α = 0.0041113, round-trip cost c = 2·1bp = 0.0002 in log-return. Expected per-trade gross log return:
  ...
  Round-009 empirical hit_rate=0.525 at val-tau=0.44 under SYMMETRIC barriers (TP=+α, SL=−α, timeout mixed). The symmetric hit_rate fraction *includes* SL losses, so the equivalent **p_TP under symmetric** is approximately `n_tp / n_trades`, which Phase A round-012's headline JSON has but did not table — extracting it is a 1-line script. **Empirically, if `p_TP_symmetric` ≥ ~30% at val-τ=0.44**, the no-stop variant comfortably exceeds the 14.1% bar and should produce positive test Sharpe; if `p_TP_symmetric` < 14% the no-stop variant still loses. The ratio between symmetric `p_TP` and no-stop `p_TP` is ≈ 1 (both measure "did upper barrier hit within M=20 bars"); H-024's run will produce the headline number directly.
...
*Refinement (round-014, addendum from Ask 2)*: H-025 (sized via Mondrian-ACI) and H-302 (σ-epistemic *abstention*) are NOT the same hypothesis and must not be conflated. **H-025** *scales* position size by a per-regime confidence (`size = clip(k · max(0, p − q_t), 0, 1)`) — every above-threshold opportunity is taken, only at differing magnitude. **H-302** *gates* trade entry by a CatBoost-virtual-ensemble σ_epistemic — when σ is above a val-chosen threshold, the trade is skipped entirely (size = 0) regardless of p. The two interact: H-025 uses the conformal q_t (a coverage-derived signal that includes both aleatoric and epistemic noise); H-302 isolates the epistemic component (model leverage, low confidence ≠ noisy data). Empirically the disagreement region is high-aleatoric-low-epistemic — exactly where the offline CatBoost is confident but the underlying DGP is genuinely random; H-025 will trade there, H-302 will not. Phase A round-012 already showed `mondrian_aci_size` (H-025-shaped) at k\*=20 producing test Sharpe=−1.51 with p_boot=0.49 (no edge after deflation), making H-302's separate test essential before any joint H-025+σ variant. Run H-302 first; only if it shows σ-conditional Sharpe lift does the H-025+H-302 combination become a separate card.
...
- **Status**: blocked on H-113. *Refinement (round-014)*: superseded for τ-grid CSCV by Phase A round-013 work landing `cscv_pbo` in `src/backtest.py`; H-005c now collapses to "use the new helper to recompute the round-001 PBO", purely a regression check.
- **Cost**: low (post-H-113)
...
- **Mechanism**: load `artifacts/offline_model/rolling/predictions.parquet` (built by H-310-a); re-run the Phase-A unified harness (`scripts/phase_A_round_011.py` adapted) producing baseline_offline_tau, combined_avg_tau, combined_stacked_tau, conformal_gate_tau, mondrian_aci_size headline numbers under rolling-retrain. Diff against the static-train numbers in `RESEARCH/diagrams/phase_A/round_012/phase_A_partial_v2.csv` row-by-row. Report per-strategy ΔSharpe, ΔBSS, Δper-regime-ECE, paired bootstrap p (H-320-a stationary block bootstrap on per-bar net log returns, block length via Politis-White). Multi-strategy DSR (Bailey-LdP) with `n_trials = 7 strategies × 26 τ-grid` deflation, applied to the rolling-retrain run as a self-contained Phase-A reproduction.
- **Predicted effect**: ΔSharpe ∈ [-0.05, +0.20] for `baseline_offline_tau` vs static; ΔBSS ∈ [+0.005, +0.02]; the high-vol tercile's ΔECE expected to be the largest single gain. Phase A's null result (multi-strategy DSR=0 for all 7) might or might not flip — the falsifier specifies the threshold.
...
- **Mechanism**: a single new module `src/bootstrap.py` exposes one function per metric, each returning `(point_estimate, ci_lo, ci_hi, B, scheme_name, block_length_or_none)`. The scheme matrix is fixed by the metric's sampling distribution, not chosen — see card body in `RESEARCH/research_plan_round_014.md` §Ask 8 Matrix. Concretely:
  ...
  | Cross-strategy SR | `cross_strategy_sr` | Already implemented: `cscv_pbo` (round-012) + `deflated_sharpe` (round-001) |
...
- **Mechanism**: re-render every metric in `RESEARCH/REPORT.md` with the corresponding `src/bootstrap.py` CI: round-005 calibration plot per regime (per-bin Wilson + outer bootstrap); round-001/round-009 Sharpe (stationary block bootstrap CI vs the existing point estimate); round-007/round-008 coverage gap (per-regime stratified bootstrap on the (predicted, observed) pair); Phase A round-012 multi-strategy DSR (recomputed with paired block-bootstrap on per-strategy per-bar returns). Diff against the bare numbers; flag any "accepted" claim whose 95% CI contains the null. The deliverable is one updated REPORT.md rendering pass + one `RESEARCH/diagrams/round_NNN/audit_diff.csv` with one row per audited metric: `(round_id, metric, point, ci_lo, ci_hi, scheme, contains_null)`.
...
- **Mechanism**: take the val-chosen offline strategy (`baseline_offline_tau` τ\*=0.44 from Phase A) as the base; layer a Cantelli/Chebyshev-style abstention rule on top using `predict_with_decomposed_uq` (already in `src/uncertainty.py`): drop trades where `p_offline − k · σ_epistemic < τ`, i.e., the lower-bound on `P(y=1)` does not clear the strategy threshold. Sweep `k ∈ {0, 0.5, 1.0, 1.5, 2.0}` on **validation Sharpe** (n_trades ≥ 50 stability filter); apply the val-chosen `(τ\*, k\*)` to test once. Multi-comparison deflation (Bailey-Borwein DSR) over the (τ × k) = 26 × 5 = 130-trial grid; CSCV PBO over the k-grid using H-320-a's bootstrap routines. The hypothesis: σ_epistemic is informative for *abstention* under regime drift (high σ marks model leverage, where the offline calibration is least trustworthy — round-005 high-vol ECE = 0.171). A disagreement diagnostic: scatter `(p_offline, σ_epistemic)` colored by realized y on test and check whether the abstained region has higher Brier than the kept region (kept-vs-abstained Brier ratio < 0.85 = signal). The offline ensemble currently lives as a single `model.cbm` not a 3-seed ensemble — this card requires materialising a 3-seed CatBoostEnsemble per H-108 first (already accepted), or accepting the σ_epistemic≈0 fallback that `_td_predict_single` produces under SGLB-disabled training (unsuitable; we re-train under `langevin=True` as a precondition).
- **Predicted effect**: kept-vs-abstained Brier ratio in [0.80, 0.95] in the high-vol tercile (where round-005 ECE was 0.171); kept-region test Sharpe shift of +0.05 to +0.20 vs `baseline_offline_tau` if σ is informative; multi-strategy DSR > 0.95 only if k\* ≠ 0 and the lift survives 130-trial deflation. Seed-noise band ≈ 0.02 Sharpe (CatBoost virtual-ensemble σ has its own seed sensitivity — k\* shifts ±1 step under reseed are common). Below this band → H-302 is null.
- **Falsification**: at val-chosen `(τ\*, k\*)`, test Sharpe lift over `baseline_offline_tau` < 0.05 AND kept-vs-abstained Brier ratio in (0.95, 1.05) → σ_epistemic is uninformative for abstention; H-302 is null. Additionally if k\*=0 wins the val grid, σ_epistemic carries no actionable signal at any threshold → null.
- **References**:
...
- **Status**: ACCEPTED (round-012, Phase A r2). `cscv_pbo` lives in `src/backtest.py`; 4 tests pass including pure-noise PBO ∈ [0.35, 0.75] and clear-winner PBO < 0.05; 12,870 combinations on n_chunks=16. Phase A round-013 wires the cross-strategy + per-strategy CSCV passes onto the existing accept-gate machinery.
- **Cost**: medium
