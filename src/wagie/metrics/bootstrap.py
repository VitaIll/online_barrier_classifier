"""Bootstrap CIs for time-series + classification statistics.

Ported from `src/bootstrap.py` (round-020, 470 LoC + 30 tests). The original
file was deleted when the package collapsed into `wagie`; this is the
canonical home in the new layout.

Implements:
- Politis & Romano (1994) JASA 89:1303 stationary block bootstrap.
- Politis & White (2004) Econometric Reviews 23:53 flat-top plug-in for
  automatic block-length selection (Patton-Politis-White 2009 corrigendum).
- DeLong (1988) closed-form ROC-AUC variance, Sun-Xu (2014) IEEE SPL 21:1389
  O((n+m) log(n+m)) reformulation.
- Wilson score interval — closed-form binomial CI matching
  `statsmodels.stats.proportion.proportion_confint(method="wilson")`.
- Boyd-Eng-Page (2013) stratified PR-AUC bootstrap.
- AFML Chapter 4.4 trade-concurrency / average-uniqueness.
- AFML Snippet 4.5 sequential bootstrap for overlap-aware per-trade resampling.
...
Hadamard-differentiable functionals. Max drawdown is a sup statistic and is
NOT Hadamard-differentiable in the empirical CDF — coverage degrades when
ρ̂(1) is mis-estimated under heavy tails.

Only numpy + scipy required. `stratified_bootstrap_pr_auc` optionally pulls
in scikit-learn; the rest of the public surface is dependency-light.
"""
...
def optimal_block_length(
    ...
    if not np.isfinite(b_opt) or b_opt < 1.0:
        return 1
    # Politis-White robustness: cap at n / 4 to avoid degenerate single-block
    # bootstrap.
...
# -----------------------------------------------------------------------------
# Specialized wrappers (Sharpe / Sortino / max DD)
# -----------------------------------------------------------------------------
...
def bootstrap_max_drawdown(returns: np.ndarray, **kwargs) -> dict:
    ...
    Known caveat: max-DD is a path functional (sup statistic), NOT
    Hadamard-differentiable in the empirical CDF — coverage degrades
...
# -----------------------------------------------------------------------------
# Per-bin Wilson interval (binomial CI for calibration bins)
# -----------------------------------------------------------------------------
...
def wilson_interval(
    ...
    Per-bin calibration CI uses Wilson rather than bootstrap because per-bin
    n is small; Wilson is closed-form on the binomial likelihood and matches
    `statsmodels.stats.proportion.proportion_confint` to 1e-12 in tests.
    """
...
# -----------------------------------------------------------------------------
# DeLong (1988) closed-form ROC-AUC variance
# -----------------------------------------------------------------------------
...
def delong_roc_auc_ci(
    ...
    # Per Sun-Xu eq (5)-(7) — fast computation via ranks within own class:
...
# -----------------------------------------------------------------------------
# Stratified bootstrap for PR-AUC
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Per-trade overlap-aware bootstrap (AFML §4.4–4.5)
# -----------------------------------------------------------------------------
