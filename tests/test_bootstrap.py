...
- Stationary-block bootstrap matches IID bootstrap CI within 5% on IID data.
- Stationary-block bootstrap CI widens with autocorrelation on AR(1).
- DeLong ROC-AUC CI matches IID bootstrap to within 0.005 width on synthetic
  balanced sample.
- Wilson interval matches statsmodels.stats.proportion.proportion_confint
  at the binomial CI to 1e-12.
- Stratified PR-AUC bootstrap returns finite CIs on standard inputs.
- Sequential bootstrap with overlap-aware weights produces wider CIs than
  naive when overlap is high.
"""
...
def test_optimal_block_length_ar1_returns_larger_than_iid():
    """On AR(1) with rho=0.8, block length should be substantially larger than IID."""
    rng = np.random.default_rng(0)
    n = 5000
    rho = 0.8
    ...
    for i in range(1, n):
        x[i] = rho * x[i - 1] + eps[i]
    bl = optimal_block_length(x)
    # AR(1) with rho=0.8 has effective sample size much smaller; bl should be > 5.
...
def test_block_bootstrap_block_length_1_is_iid():
    ...
    # IID should have very low autocorrelation in indices.
    diff = np.diff(idx).astype(float)
    var_diff = diff.var()
    var_iid = (n ** 2 - 1) / 6.0  # variance of |U(0,n) - U(0,n)| approx
    assert var_diff > 0.0
...
def test_bootstrap_sharpe_iid_matches_iid_bootstrap_within_5pct():
    """On IID data, stationary-block-bootstrap CI should match IID bootstrap CI
    within 5% width (Politis-Romano §3 — IID is a degenerate case of stationary
    block at p ≈ 1)."""
    rng = np.random.default_rng(0)
    n = 2000
    ...
    assert rel_diff < 0.10, (
        f"expected stationary-block CI width to match IID within 10% on IID data; "
        f"got rel_diff={rel_diff:.3f}, block={width_block:.4f}, iid={width_iid:.4f}"
    )


def test_bootstrap_sharpe_ar1_widens_correctly():
    """On AR(1) data, stationary-block-bootstrap CI should be WIDER than IID
    bootstrap CI (the IID-bootstrap under-covers under positive autocorrelation)."""
    rng = np.random.default_rng(0)
    n = 5000
    rho = 0.7
...
def test_wilson_matches_statsmodels_to_1e12():
    """Per H-191 spec: Wilson must match statsmodels proportion_confint to 1e-12."""
...
def test_delong_roc_auc_balanced_sample_matches_bootstrap_within_0005():
    """Per H-191 falsifier: synthetic IID DeLong CI half-width matches
    IID bootstrap CI half-width within 0.005 on a balanced 50k-sample
    experiment."""
    rng = np.random.default_rng(42)
    n = 5000  # smaller for test speed; the bound still holds
    ...
    # IID bootstrap of AUC for comparison.
    ...
    # On n=5000 with this signal, both half-widths are ~0.01.
    # Permit 0.005 absolute as per H-191 falsifier wording.
    assert diff < 0.005, (
        f"DeLong half-CI ({half_delong:.4f}) vs bootstrap half-CI "
        f"({half_boot:.4f}) differ by {diff:.4f}; H-191 threshold is 0.005"
    )
...
def test_delong_rejects_single_class():
    with pytest.raises(ValueError):
        delong_roc_auc_ci(np.zeros(100, dtype=int), np.random.uniform(size=100))
    with pytest.raises(ValueError):
        delong_roc_auc_ci(np.ones(100, dtype=int), np.random.uniform(size=100))
...
def test_delong_returns_documented_keys():
    ...
    out = delong_roc_auc_ci(y_true, y_score)
    for key in ["estimate", "ci_lo", "ci_hi", "se", "scheme", "n_pos", "n_neg",
                "confidence"]:
        assert key in out
...
def test_stratified_pr_auc_rejects_single_class():
    with pytest.raises(ValueError):
        stratified_bootstrap_pr_auc(np.zeros(100, dtype=int), np.random.uniform(size=100))
...
def test_overlap_aware_bootstrap_widens_ci_under_high_overlap():
    """Under high trade overlap, sequential-bootstrap CI should be wider than
    naive trade-level bootstrap CI (per H-193 expected effect)."""
    rng = np.random.default_rng(42)
    n_trades = 500
    # All trades start sequentially every minute, hold for 30 minutes — heavy overlap.
    ...
    # Sequential CIs should be at least 90% as wide as naive (sometimes wider,
    # sometimes similar — but never collapsed).
...
def test_overlap_aware_bootstrap_no_overlap_matches_naive():
    ...
    # Mean uniqueness should be 1.0 (no overlap).
...
def test_overlap_aware_bootstrap_returns_documented_keys():
    ...
    for key in ["estimate", "ci_lo", "ci_hi", "se", "scheme", "n_resamples",
                "n_trades", "mean_uniqueness", "median_uniqueness", "confidence"]:
        assert key in out
